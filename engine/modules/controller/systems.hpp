...
namespace zef {
  namespace sys {
    ...
    void handleclickable(zef::Engine& engine,
                         ecs::sparse_array<zef::comp::clickable>& clickables,
      ...
      for (auto&& [i, clk, pos] : ecs::indexed_zipper(clickables, positions)) {
        for (auto& hitboxe : clk._hitboxes) {
          ...
          if (hitboxe.isColliding(mouse)) {
            ...
            if (engine.getUserInputs().mouse._mb_map.at(
                    zef::utils::MouseButtons::Left)) {
