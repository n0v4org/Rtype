...
#ifndef ENGINE_MODULES_COLLISION_UTILS
    ...
    public:
      ...
      bool isColliding(const hitbox &other) {
        ...
        if (_type == CIRCLE && other._type == CIRCLE) {
          float dist =
              sqrt(pow(_posX - other._posX, 2) + pow(_posY - other._posY, 2));
          collide = dist < (_radius + other._radius);
        } else if (_type == CIRCLE && other._type == RECT) {
          float closestX = std::max(
              other._posX, std::min(_posX, other._posX + other._width));
          float closestY = std::max(
              other._posY, std::min(_posY, other._posY + other._height));
          float sqDistance =
              pow(closestX - _posX, 2) + pow(closestY - _posY, 2);
          collide = sqDistance < pow(_radius, 2);
        } else if (_type == RECT && other._type == CIRCLE) {
          float closestX =
              std::max(_posX, std::min(other._posX, _posX + _width));
          float closestY =
              std::max(_posY, std::min(other._posY, _posY + _height));
          float sqDistance =
              pow(closestX - other._posX, 2) + pow(closestY - other._posY, 2);
