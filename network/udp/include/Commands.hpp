...
static input_t unpack(std::size_t byte_size, std::array<uint8_t, 1024 > _recv_buffer_) {
          input_t input = {0};
    ...
    private:
        ...
        void parsePayload(const input_t& input) {
            ...
            if (input.payload_size != dataSize) {
                throw std::runtime_error("Payload size does not match expected structure size");
            }
