...
private:
    ...
    std::string compressString(const std::string& input) {
        ...
        int compressedSize = LZ4_compress_default(
            input.c_str(), compressedData.data(), input.size(), maxCompressedSize);
    ...
    std::string decompressString(const std::vector<char>& compressed, uint32_t original_size) {
        std::vector<char> decompressedData(original_size);

        int decompressedSize = LZ4_decompress_safe(
            compressed.data(), decompressedData.data(), compressed.size(), original_size);

        if (decompressedSize < 0) {
            throw std::runtime_error("Decompression failed with error code: " + std::to_string(decompressedSize));
        }
