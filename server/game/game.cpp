...
void runServer(int port) {
  ...
  engine.registerCommand(CONNECT, [](zef::Engine &engine, input_t input) {
    ...
    for (auto &i : engine._server->getAllIds()) {
      if (i != input.id) {
        engine.ServerSend<CommandSpawnAlly>(
            i, SPAWNALLY, {0.0f, 0.0f, static_cast<size_t>(input.id)});
      }
    }
    ...
    for (auto &&[pl, pos, rep] :
      ...
      if (rep._id != input.id)
        engine.ServerSend<CommandSpawnAlly>(input.id, SPAWNALLY,
                                            {pos.x, pos.y, rep._id});
    }
  ...
  engine.registerCommand(MOVEPLAYER, [](zef::Engine &engine, input_t input) {
    ...
    for (auto &&[p, rep] :
         ecs::zipper(engine.reg.get_components<Player>(),
                     engine.reg.get_components<zef::comp::replicable>())) {
      if (rep._id != input.id) {
        engine.ServerSend<CommandMoveAlly>(rep._id, MOVEALLY,
  ...
  engine.registerCommand(SHOOTPLAYER, [](zef::Engine &engine, input_t input) {
    ...
    for (auto &&[rep, player] :
         ecs::zipper(engine.reg.get_components<zef::comp::replicable>(),
  ...
  engine.registerCommand(SENDPOSITION, [](zef::Engine &engine, input_t input) {
    CommandSendPosition cmp =
        network::game::Commands<CommandSendPosition>(input).getCommand();
    for (auto &&[pl, rep, pos] :
         ecs::zipper(engine.reg.get_components<Player>(),
                     engine.reg.get_components<zef::comp::replicable>(),
                     engine.reg.get_components<zef::comp::position>())) {
  ...
  // engine.addSystem<zef::comp::collidable,
  // zef::comp::position>(zef::sys::check_collidables);
