...
TEST_F(NetworkTest, BasicSerializationAndDeserialization) {
  ...
  network::game::Commands<test_1_t> cmd_handler =
      network::game::Commands<test_1_t>(parsed_payload);
...
TEST_F(NetworkTest, BigDataSerialization) {
  ...
  input_t parsed_payload = network::game::unpack(sizeof(message), message);
  network::game::Commands<test_2_t> cmd_handler =
      network::game::Commands<test_2_t>(parsed_payload);
