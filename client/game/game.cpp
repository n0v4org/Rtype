...
void runClient(int sport, int cport, std::string ip) {
    ...
    engine.registerCommand(SETPLAYERPOS, [](zef::Engine& engine, input_t input) {
        CommandSetPlayerPos spp = network::game::Commands<CommandSetPlayerPos>(input).getCommand();
    ...
    engine.registerCommand(SETALLYPOS, [](zef::Engine& engine, input_t input) {
        CommandSetAllyPos sap = network::game::Commands<CommandSetAllyPos>(input).getCommand();
        for (auto && [rep, pos] : ecs::zipper(engine.reg.get_components<zef::comp::replicable>(), engine.reg.get_components<zef::comp::position>())) {
            if (sap.rep == rep._id) {
                pos.x = sap.x;
                pos.y = sap.y;
            }
    ...
    engine.registerCommand(ASKPOSITION, [](zef::Engine& engine, input_t input) {
        ...
        for (auto &&[pl, pos, vec] : ecs::zipper(engine.reg.get_components<Player>(), engine.reg.get_components<zef::comp::position>(), engine.reg.get_components<zef::comp::vector>())) {
            engine.ClientSend<CommandSendPosition>(SENDPOSITION, {pos.x, pos.y, vec.x, vec.y});
        }
