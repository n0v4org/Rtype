...
public:
    static void instanciate(zef::Engine& engine,
                            ...
                            float offsetY = 0.f)
    {
        parseAndCreateEntities(engine, configPath, offsetX, offsetY);
    }

private:
    static void parseAndCreateEntities(zef::Engine& engine,
                                       const std::string& configPath,
                                       float offsetX,
                                       float offsetY)
    {
        ...
        while (std::getline(file, line)) {
            ...
            if (type.rfind("Wall", 0) == 0) {
                ...
                parseRepeatWall(engine, type, iss, line, offsetX, offsetY);
            }
            else if (type == "Plane") {
                float x, y;
                if (!(iss >> x >> y)) {
    ...
    static void parseRepeatWall(zef::Engine& engine,
